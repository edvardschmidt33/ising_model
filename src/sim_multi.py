...
def single_run(L, J_list, T_list, eq_limit, mc_sweeps, seed=None):
    """
    Perform one complete temperature sweep and return observables.
    """
    if seed is not None:
        np.random.seed(seed)
...
if __name__ == "__main__":
    ...
    # Optional: standard deviation between runs
    ...
    if args.io:
        with open(f"./data/results_task1_L{L}_avg.json", "w") as f:
    ...
    if args.plot:
        ...
        f.suptitle(f"Metropolis algorithm with L = {L} ({runs} runs averaged)", fontsize=25)
        ...
        plt.savefig(f"./figs/Task1_{L}x{L}_avg")
        plt.show()
